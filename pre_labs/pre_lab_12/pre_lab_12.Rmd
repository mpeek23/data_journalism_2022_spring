...
**Answer**
The dataset loads the number of vaccinations for every day and some additional information about them, like if they're the first or second dose, vaccination status, etc.
```{r}
...
**Answer**
It has a NA category with a ton of vaccinations.
...
**Answer**
It shows all the data points with NA for the county.
...
**Answer**
It ranks the number of vaccinations for each day. There seems to be more lower-rank days in the past few days.
```{r}
...
**Answer**
The bar chart shows that there was ebb and flow in the number of vaccinations.
```{r fig.width=9}
ggplot() +
...
**Answer**
[1] "The number of people in Prince George's County who got fully vaccinated on March 06 decreased by -73.77 percent compared to the week before"
```{r}
...
**Answer**
The chart shows the nymber of COVID cases in Maryland counties over time.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
It makes the Maryland county COVID cases in a log scale instead
```{r}
...
**Answer**
COVID cases by Maryland county over time, updated to today!
```{r}
ggplot() +
