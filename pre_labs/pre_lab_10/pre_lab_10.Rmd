...
https://datawrapper.dwcdn.net/nDyMP/1/ 
...
Made a new data frame that groups the PPP loans by county and adds up the total number of loans.
...
**Answer** Describe what the code below is doing and the output.
Getting rid of the extraneous records by including records that are greater than 6 (the highest number of extraneous counties)
...
It adds the state fips code and the county fips code into one string, the 5 digit fips code that R is expecting.
...
Both data sets with the list of counties, its fips code and the corresponding number of PPP loans.
...
It went from a big blank US map to a little Maryland map.
...
**Answer** Describe the changes from the previous map
It gives the more descriptive label of the county instead of the fips code and adds a comma to the total.
...
https://datawrapper.dwcdn.net/JNZ1y/1/
