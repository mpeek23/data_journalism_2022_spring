...
**Answer**
It makes a data frame of state, what percent voted Trump in 2016 and PPP loan figures
```{r}
...
**Answer**
It shows some positive correlation, which could be evidence of a relationship
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
It  shows a slightly less concentrated positive correlation.
```{r}
...
**Answer**
It confirms the charts' data- the correlation is stronger for business and slightly less strong for employees.
```{r}
reveal_data %>%
...
**Answer**
It provides the statistical inputs for finding the p-value of correlation. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
It runs the same information as above (correlation and p-value) for the amount per employee.
```{r}
...
**Answer**
The table compares Clinton and Trump voters average amount per employee and average number of  businesses receiving loans.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
The statistical formula sets up a two sample t-test, showing us the means of the two groups, the confidence interval, and the t and p value.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
This looks at the t-test for businesses receiving loans, and provides the same information as above, like the confidence intervals, t and p value, etc.
```{r}
