...
**Answer**
The dataframe is 152,584 rows of Tweets with the tweet itself, the username, when it was Tweeted, and the information of the Twitter user- name, position, state and party.
```{r}
...
**Answer**
It pulls out the unique instances of the words and gets rid of the repeats.
```{r}
...
**Answer**
It pulls out all the unique words in the dataframe- there are a lot.
```{r}
...
**Answer**
It makes a graphic of all the words used and counts them, sorts them by most to least, etc. It is not very interesting, it has a lot of throwaway words.
...
**Answer**
By using antijoin, we get only the unique words we want and get rid of the boring words. It produces a tibble of more interesting words, like covid, health, pandemic etc. It's not perfect, it gives us not interesting things like 19 or amp.
```{r}
unique_words %>%
...
**Answer**
It creates two dataframes of 2020's top 10 words and 2021's top 10 words. The biggest difference was the vaccines and vaccine rocketed up the list in 2021.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
It creates a dataframe with the top 10 most used two word phrases in 2020- you see covid 19, health care, public health, etc.
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
This creates a dataframe of the top 10 two word phrases in 2021- you see covid 19, 19 vaccine, covid relief, lots more vaccines, etc.
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
This creates two dataframes of Democrat and Republican tweets in 2020 and the top 10 two phrases. Democrats have "covid 19" "health care" "town hall" "social distancing" etc. and tweeted a lot. Republicans had similar phrases- "covid 19" "coronavirus pandemic" "cares act" "american people" and tweeted a lot less.
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
This creates two dataframes of Republicans top two phrases in 2020 and 2021. In 2020, the top words were "covid 19", "president trump", "coronavirus pandemic" or "nursing home." In 2021, the top words were "covid 19" "covid relief" or "covid vaccine."
```{r}
covid_tweets %>%
...
**Answer**
Some sentiments found by the code in 2020 and 2021 seemed a little off, while some seemed accurate. Words like relief, support, thank or safe seem safe to call a positive sentiment, while virus or emergency feel solidly negative. Other words, like Trump or like, I think can't be assigned a particular sentiment because they're not real subject words.
```{r}
