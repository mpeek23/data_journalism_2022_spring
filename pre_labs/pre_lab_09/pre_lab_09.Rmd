...
**Answer**
1: Loads the data and gives it a nice name, maryland_ppp_top_counties
2: Groups the data by project county names
3: Summarises the total number of loans
4: Arranges the counties by the total number of loans, and puts them in descending order
5: Picks the top 10 counties
```{r}
...
**Answer**
It makes an empty box that we can fill with code, visualized!
```{r}
maryland_ppp_top_counties %>%
  ggplot()
```
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
This code creates a bar chart of the data. It's very basic.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
The graph is re-ordered to have the least count to the greatest count of loans.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The code coord_flip is added, which flips the axis of the chart.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The code adds theme_minimal, which changes the style of the chart to make the background white and defines the axis lines more clearly.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer**
Downloads a new library, ggthemes
```{r}
...
**Answer**
It makes the background blue, changes the font, and changed the way the axis are drawn.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The code added new labels, including a new title, changes the x and y axis labels, and adding a source.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
It creates a data set with date approved and the total amount of loans.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
It creates a line chart of the dates mapped with the toal loans. The data is high in the beginning dates, flattens out and then peaks a little bit.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The x axis has more labels to map more specific months.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The code tilts the date labels sideways so they fit better and we can actually read them!
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
